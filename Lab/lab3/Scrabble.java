...
public class Scrabble {
    // wypelnianie tablicy 1000 slow angielskich
    //public static ArrayList<String> englishWords = new ArrayList<>();
    public static ArrayList<String> englishWords = new ArrayList<>(Arrays.asList(
            ...
            "international", "interview", "into", "investment", "involve", "issue", "it",
            "item", "its", "itself", "job", "join", "just", "keep", "key", "kid", "kill",
            "kind", "kitchen", "know", "knowledge", "land", "language", "large", "last",
            "late", "later", "laugh", "law", "lawyer", "lay", "lead", "leader", "learn",
            "least", "leave", "left", "leg", "legal", "less", "let", "letter", "level",
            "lie", "life", "light", "like", "likely", "line", "list", "listen", "little",
            ...
            "more", "morning", "most", "mother", "mouth", "move", "movement", "movie", "mr",
            "mrs", "much", "music", "must", "my", "myself", "name", "nation", "national",
            "natural", "nature", "near", "nearly", "necessary", "need", "network", "never",
            "new", "news", "newspaper", "next", "nice", "night", "no", "none", "nor", "north",
            "not", "note", "nothing", "notice", "now", "nt", "number", "occur", "of", "off",
            ...
            "risk", "road", "rock", "role", "room", "rule", "run", "safe", "same", "save", "say",
            ...
            "sign", "significant", "similar", "simple", "simply", "since", "sing", "single", "sister",
            ...
            "stock", "stop", "store", "story", "strategy", "street", "strong", "structure", "student",
            ...
            "throughout", "throw", "thus", "time", "to", "today", "together", "tonight", "too",
            "top", "total", "tough", "toward", "town", "trade", "traditional", "training", "travel",
    ...
    public static Map<Character, Integer> wordToMap(String word) {
        ...
        for(int i = 0; i < word.length(); i++) {
            amount = wordMap.getOrDefault(word.charAt(i), 0);
            wordMap.put(word.charAt(i), amount + 1);
        }
        return wordMap;
    }
    public static void main(String[] args) {
        ...
        // zamiana angielskich slow na tablice slownikow
        Collections.sort(englishWords, Comparator.comparingInt(String::length).reversed());
//        System.out.println("Najdluzsze slowo: " + englishWords.get(0));
//        System.out.println("Najkrotsze slowo: " + englishWords.get(999));
        ArrayList<Map<Character, Integer>> englishWordsArrayOfMaps = new ArrayList<>();
        for(int i = 0; i < 1000; i++) {
            englishWordsArrayOfMaps.add(wordToMap(englishWords.get(i)));
        }

        // wczytywanie
        Scanner scanner = new Scanner(System.in);
        System.out.print("Podaj liczbe liter na reke gracza: ");
        int lettersAmount = scanner.nextInt();
        //int lettersAmount = 20;
        char[] letters = new char[lettersAmount];
        System.out.println();

        // tworzenie 3 graczy
        Player Player1 = new Player(lettersAmount);
        Player Player2 = new Player(lettersAmount);
        Player Player3 = new Player(lettersAmount);

        // wypsiywanie poczatkowych liter na rekach graczy
        System.out.print("Litery poczatkowe na rece gracza 1: "); Player1.printHand();
        System.out.print("Litery poczatkowe na rece gracza 2: "); Player2.printHand();
        System.out.print("Litery poczatkowe na rece gracza 3: "); Player3.printHand();
        System.out.println();

        // symulacja gry
        boolean ifWordWasCreated = true;
        while(ifWordWasCreated) {
            ifWordWasCreated = false;
            for(int i = 0; i < englishWords.size(); i++) {
                if(Player1.isAbleToCreate(englishWordsArrayOfMaps.get(i), englishWords.get(i))) {
                    ifWordWasCreated = true;
                    englishWords.remove(englishWords.get(i));
                    englishWordsArrayOfMaps.remove(i);
                    break;
                }
            }

            for(int i = 0; i < englishWords.size(); i++) {
                if(Player2.isAbleToCreate(englishWordsArrayOfMaps.get(i), englishWords.get(i))) {
                    ifWordWasCreated = true;
                    englishWords.remove(i);
                    englishWordsArrayOfMaps.remove(i);
                    break;
                }
            }
            ...
            for(int i = 0; i < englishWords.size(); i++) {
                if(Player3.isAbleToCreate(englishWordsArrayOfMaps.get(i), englishWords.get(i))) {
                    ifWordWasCreated = true;
                    englishWords.remove(i);
                    englishWordsArrayOfMaps.remove(i);
                    break;
        ...
        // sortowanie alfabetyczne slow stworzonych przez graczy
        Collections.sort(Player1.usedWords);
        Collections.sort(Player2.usedWords);
        Collections.sort(Player3.usedWords);

        // wypisywanie slow stworzonych przez graczy
        System.out.print("Slowa stworzone przez gracza1: "); Player1.printUsedWords();
        System.out.print("Slowa stworzone przez gracza2: "); Player2.printUsedWords();
        System.out.print("Slowa stworzone przez gracza3: "); Player3.printUsedWords();
        System.out.println();

        // wypsiywanie liter na rekach graczy po zakonczeniu gry
        System.out.print("Litery koncowe na rece gracza 1: "); Player1.printHand();
        System.out.print("Litery koncowe na rece gracza 2: "); Player2.printHand();
        System.out.print("Litery koncowe na rece gracza 3: "); Player3.printHand();
        System.out.println();

        // czas wykonania programu w milisekundach
